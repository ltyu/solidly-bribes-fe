...
class Store {
  ...
  getPairByAddress = async (pairAddress) => {
    try {
      ...
      const [token0Symbol, token0Decimals, token0Balance, token1Symbol, token1Decimals, token1Balance] =
        await Promise.all([
  ...
  _getBaseAssets = async () => {
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_API}/api/baseAssets`, {
        method: 'get',
        // headers: {
  ...
  _getRouteAssets = async () => {
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_API}/api/routeAssets`, {
        method: 'get',
        // headers: {
        //   Authorization: `Basic ${process.env.NEXT_PUBLIC_API_TOKEN}`,
        // },
  ...
  _getVestNFTs = async (web3, account) => {
    try {
      ...
      const v2LockedContract = new web3.eth.Contract(CONTRACTS.V2_VOTER_ABI, CONTRACTS.V2_VOTER_ADDRESS)
      const v2LockedLength = await v2LockedContract.methods.balanceOf(account.address).call()

      const v2Arr = Array.from({ length: parseInt(v2LockedLength) }, (v, i) => i)

      const v2Nfts = await Promise.all(
        v2Arr.map(async (idx) => {
          const tokenIndex = await v2LockedContract.methods.ownerToNFTokenIdList(account.address, idx).call()
          const locked = await vestingContract.methods.locked(tokenIndex).call()
          const lockValue = await vestingContract.methods.balanceOfNFT(tokenIndex).call()
  ...
  _getPairInfo = async (web3, account, overridePairs) => {
    try {
      const start = moment()
      const pairs = overridePairs ? overridePairs : this.getStore('pairs')
      if (pairs.length > 0) {
        const gaugesContract = new web3.eth.Contract(CONTRACTS.VOTER_ABI, CONTRACTS.VOTER_ADDRESS)
        ...
        this.emitter.emit(ACTIONS.UPDATED)
        this.emitter.emit(ACTIONS.PAIR_INFO_UPDATED)
      }
    } catch (ex) {
      console.log(ex)
    }
            ...
            async (err) => {
              ...
              await new Promise((resolve, reject) =>
                this._callContractWait(
                  ...
                  async (err) => {
                    if (err) {
                      this.emitter.emit(ACTIONS.ERROR, err)
                      reject(err)
                    }
  ...
  _getApprovedV2Voter = async (web3, tokenID) => {
    try {
      const tokenContract = new web3.eth.Contract(CONTRACTS.VE_TOKEN_ABI, CONTRACTS.VE_TOKEN_ADDRESS)
      const approved = await tokenContract.methods.isApprovedOrOwner(CONTRACTS.V2_VOTER_ADDRESS, tokenID).call()
      return approved
    } catch (ex) {
      console.error(ex)
      return null
  ...
  _getTransferredV2Voter = async (web3, tokenID, account) => {
    try {
      const voterContract = new web3.eth.Contract(CONTRACTS.V2_VOTER_ABI, CONTRACTS.V2_VOTER_ADDRESS)
      const owner = await voterContract.methods.nftOwner(tokenID).call()
  ...
  vote = async (payload) => {
    try {
      ...
      const approveTXID = this.getTXUUID()
      const transferTXID = this.getTXUUID()

      this.emitter.emit(ACTIONS.TX_ADDED, {
        title: `Locking vote using token #${tokenID}`,
        verb: 'Votes Cast',
        transactions: [
          {
            uuid: approveTXID,
            description: `Checking if NFT is approved to contract`,
            status: 'WAITING',
          },
          {
            uuid: transferTXID,
            description: `Checking if NFT is transferred to contract`,
            status: 'WAITING',
          },
          {
            uuid: voteTXID,
      ...
      const isApproved = await this._getApprovedV2Voter(web3, tokenID)
      const isTransferred = await this._getTransferredV2Voter(web3, tokenID, account)
      const gasPrice = await stores.accountStore.getGasPrice()
      ...
      if (isApproved) {
        this.emitter.emit(ACTIONS.TX_STATUS, {
          uuid: approveTXID,
          description: `Approved NFT #${tokenID} to contract`,
          status: 'DONE',
        })
      } else {
        this.emitter.emit(ACTIONS.TX_STATUS, {
          uuid: approveTXID,
          description: `Approve NFT #${tokenID} to contract`,
        })

        // Attempt to approve
        const approvalContract = new web3.eth.Contract(CONTRACTS.VE_TOKEN_ABI, CONTRACTS.VE_TOKEN_ADDRESS)
        await new Promise((resolve, reject) => {
          this._callContractWait(
            web3,
            approvalContract,
            'setApprovalForAll',
            [CONTRACTS.V2_VOTER_ADDRESS, tokenID],
            account,
            gasPrice,
            null,
            null,
      ...
      if (isTransferred) {
        this.emitter.emit(ACTIONS.TX_STATUS, {
          uuid: transferTXID,
          description: `Transferred NFT #${tokenID} to contract`,
          status: 'DONE',
        })
      } else {
        this.emitter.emit(ACTIONS.TX_STATUS, {
          uuid: transferTXID,
          description: `Transfer NFT #${tokenID} to contract`,
        })
  ...
  createBribe = async (payload) => {
    try {
      ...
      // ADD TRNASCTIONS TO TRANSACTION QUEUE DISPLAY
      let allowanceTXID = this.getTXUUID()
      let bribeCreatedTXID = this.getTXUUID()
      ...
      this.emitter.emit(ACTIONS.TX_ADDED, {
        ...
        transactions: [
          ...
          {
            uuid: bribeCreatedTXID,
            description: `Checking if bribe exists`,
            status: 'WAITING',
          },
          {
            uuid: bribeTXID,
            description: `Adding bribe`,
            status: 'WAITING',
          },
        ],
      })
      const v2voterContract = new web3.eth.Contract(CONTRACTS.V2_VOTER_ABI, CONTRACTS.V2_VOTER_ADDRESS)
      const v2BribeAddress = await v2voterContract.methods.bribes(gauge.gauge.address).call()
      // CHECK ALLOWANCES AND SET TX DISPLAY
      const allowance = await this._getBribeAllowance(web3, asset, v2BribeAddress, account)
      ...
      if (!v2BribeAddress) {
        // Create a v2 bribe assuming non-existent
        await new Promise((resolve, reject) =>
          this._callContractWait(
            web3,
            v2voterContract,
        ...
        this.emitter.emit(ACTIONS.TX_STATUS, {
          uuid: bribeCreatedTXID,
          description: `Bribe contracted exists`,
          status: 'DONE',
        })
      }
      console.log('v2BribeAddress', v2BribeAddress)
      const bribeContract = new web3.eth.Contract(CONTRACTS.V2_BRIBE_ABI, v2BribeAddress)
      const sendAmount = BigNumber(amount)
  ...
  _getBribeAllowance = async (web3, token, v2BribeAddress, account) => {
    try {
      const tokenContract = new web3.eth.Contract(CONTRACTS.ERC20_ABI, token.address)
      const allowance = await tokenContract.methods.allowance(account.address, v2BribeAddress).call()
      return BigNumber(allowance)
        .div(10 ** token.decimals)
        .toFixed(token.decimals)
    } catch (ex) {
  ...
  claimAllRewards = async (payload) => {
    try {
      ...
      if (bribePairs.length > 0) {
        // SUBMIT CLAIM TRANSACTION
        const gaugesContract = new web3.eth.Contract(CONTRACTS.V2_VOTER_ABI, CONTRACTS.V2_VOTER_ADDRESS)
